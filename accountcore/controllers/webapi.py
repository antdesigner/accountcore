...
class WebapiController(http.Controller):
    ...
    def _build_voucher(self,autoCreate,data,user):
        ...
        for i in range(0,len(voucher['entrys'])):
            ...
            #购建每条分录的科目
            entry['account']=self._build_account(entry['account'],autoCreate)
            if 'items' in entry:
                items = entry['items']
                #购建核算项目和类别
                entry['items']=self._build_item(items,autoCreate)
                account=self.env['accountcore.account'].sudo().browse([entry['account']])
                #更新科目带的核算项目类别
    ...
    def _updataAccountItemClass(self,account,items):
        ...
        itemClasses=self.env['accountcore.itemclass'].sudo().search([('name','in',items_classNames)])
        ...
        for item in items:
            if item[1]:
                mast_item=itemClasses_updata.filtered(lambda i:i.name == item[0])
                ...
                #科目被使用过
                    ...
                    raise exceptions.UserError('科目【'+account.name+'】已经被使用过,不能添加必选核算项目')
                elif mast_item:
    ...
    def _build_account(self,accountInfo,autoCreate):
        ...
        if exist_parent_id ==0:
            ...
            #购建科目类别
            accountClass=AccountClassInfo(accountInfo['accountClass'],self.env)
            if accountClass.id==0:
                accountClass.create()
            first_account=self._build_first_account(first_accountName,number,direction,accountClass.id)
    ...
    def _check_accountInfo(self,accountInfo):
        ...
        if "number" not in accountInfo or not accountInfo['number']:
            raise exceptions.UserError('科目【'+accountInfo["name"]+'】没有提供创建其一级科目的科目编号参数:number')
        if "direction" not in accountInfo:
            raise exceptions.UserError('科目【'+accountInfo["name"]+'】没有提供创建其一级科目的科目余额方向参数:direction')
        if "accountClass" not in accountInfo or not accountInfo['accountClass']:
            raise exceptions.UserError('科目【'+accountInfo["name"]+'】没有提供的一级科目的科目类别参数:accountClass')
        if "." in accountInfo["number"] or "---" in accountInfo["number"]:
            raise exceptions.UserError('科目【'+accountInfo["name"]+'】提供的一级科目编码不能包含.和---特殊字符')
        if accountInfo['direction'] not in ['借','贷']:
            raise exceptions.UserError('科目【'+accountInfo["name"]+'】提供的一级科目余额方向只能是字符"借"或"贷"')
        return True
...
class NameId():
    ...
    def _build(self):
        '''检查名称是否存在'''
        if self.id==0:
            record = self.env[self.model_name].sudo().search([('name','=',self.name)],limit=1)
            if record:
                self.id=record.id
    @abstractmethod
...
class AccountInfo(NameId):
    ...
    def create(self,*arg,**kw):
        ...
        fatherAccount=self.env[self.model_name].sudo().browse([fatherAccountId])
...
class ItemInfo(NameId):
    ...
    def create(self,*arg,**kw):
        class_id=arg[0]
        record = self.env[self.model_name].sudo().create([{'name':self.name,'itemClass':class_id}])
        self.id=record.id
#现金流量项目类别信息
...
class CashflowInfo(NameId):
    ...
    def create(self,*arg,**kw):
        class_id=arg[0]
        record = self.env[self.model_name].sudo().create([{'name':self.name,'cashFlowType':class_id }])
        self.id=record.id
# 全局标签类别信息
...
class GlobTagInfo(NameId):
    ...
    def create(self,*arg,**kw):
        class_id=arg[0]
        record = self.env[self.model_name].sudo().create([{'name':self.name,'glob_tag_class':class_id }])
        self.id=record.id
# 凭证来源信息
